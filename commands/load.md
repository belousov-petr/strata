...
- `.strata/MANIFEST.md` present → **strata mode**. If its `layout_version` isn't `3` (e.g. a legacy `strata_version: 0.0.3` stamp), say so and point at `MIGRATIONS.md` before loading anything else.
- Legacy fingerprints (`.ai/MEMORY-MAP.md`, `docs/PROJECT-MAP.md`, `.claude/memory/`) → **legacy layout**: orient from what exists, tell the user this is a 0.0.1/0.0.2 layout, and say `strata init` will run the matching `MIGRATIONS.md` rung. Never scaffold the current layout beside it.
- Neither, but `.strata/memory/project_state.md` exists → **flat mode**: read it as the single source; say `strata init` will archive and migrate it into the current layout (`layout_version: 3`) when invoked.
