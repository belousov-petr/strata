...
├── MIGRATIONS.md                  # v1 → v2 → v3 ladder: detect, transform, rollback
...
Inventories the session (resumption point, issue events, learnings, decisions, doc impact, external completions), routes everything per the manifest, and shows one preview block before touching anything:
...
Safeguards: git-dirty files are never moved, ADR numbers are collision-checked, deletions are section-only, and re-running with no new work proposes nothing.
...
One-shot scaffold. Two questions (project name, code vs knowledge project), then the tree above, with adapters written only if absent. Idempotent; refuses to run over an existing setup; detects v1/v2 layouts and offers the migration ladder instead of double-initializing.
...
Inside the project root: `Skill(name='strata', args='init')`. Two questions, one tree, idempotent.

## Migrating from v1/v2

[`MIGRATIONS.md`](MIGRATIONS.md) is the ladder. It detects the generation by fingerprint (v1: `docs/PROJECT-MAP.md` or `.claude/memory/`; v2: `.ai/MEMORY-MAP.md`), then runs an ordered, gated transform with a rollback anchor: namespace rename, manifest rewrite, extraction of `open_action_items.md` + `project_<slug>.md` + `docs/parked/` into the issues backlog, `feedback_*` conversion into learnings, view regeneration. Content-bearing steps archive their sources before deleting anything. The old `/save-point` and `/load-point` command names are gone; install the new files per above.
...
- **The preview gate is discipline, not a shield.** Approve without reading and misclassified content moves anyway.
...
If you've run this and found gaps, I'd like to hear about it. Open an issue or PR with: what kind of project you ran it on, what the routing got wrong (or what the preview gate let through), and what you'd change.
