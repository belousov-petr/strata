...
- **The deterministic writer must not ship default-on, and `hooks/README.md` must stop describing the read-side loop and the Codex field names as live, until the prerequisites land** — all prerequisites have since landed (P1+P2+P3 shipped 2026-06-20). Phased (full detail in `docs/deterministic-capture-design.md`):
